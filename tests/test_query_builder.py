...
class TestQueryBuilderBasic:
    ...
    def test_where_clause(self):
        q = QueryBuilder().select("id").from_table("users").where("id = 1").build()
    ...
    def test_limit(self):
        q = QueryBuilder().select("id").from_table("users").limit(10).build()
        assert "LIMIT 10" in q

    def test_offset(self):
        q = QueryBuilder().select("id").from_table("users").offset(20).build()
        assert "OFFSET 20" in q

    def test_limit_and_offset(self):
        q = QueryBuilder().select("id").from_table("users").limit(10).offset(20).build()
        assert "LIMIT 10" in q
        assert "OFFSET 20" in q

    def test_order_by_asc(self):
        q = QueryBuilder().select("id").from_table("users").order_by("name").build()
...
class TestQueryBuilderReset:
    ...
    def test_reset_clears_all(self):
        q = QueryBuilder().select("id").from_table("users").where("id = 1").limit(10)
        q.reset()
        result = q.build()
